...
pros::Motor back_right_motor (RIGHT_MOTOR_PORT);
pros::Motor back_left_motor (LEFT_MOTOR_PORT);
...
void setIndividualMotor(int Right, int Left) {
    back_right_motor.move(Right);
    back_left_motor.move(Left);
}
...
void driveForDistancePID(int distance, int speed) {
  ...
  while (!((back_left_motor.get_position() < motorUpper) && (back_left_motor.get_position() > motorLower))) {
  ...
  if(DEBUG_ON){
    std::cout << "Encoder Left: " << back_left_motor.get_position();
    std::cout << " Encoder Right: " << back_right_motor.get_position() << "\n";
  }
}
...
void pivotTurn(int speed, long turnAngle) {
  ...
  if(DEBUG_ON){
    std::cout << " RESET -- Encoder Left: " << back_left_motor.get_position();
    std::cout << " Encoder Right: " << back_right_motor.get_position() << "\n";
  ...
  while ((!((fabs(back_right_motor.get_position()) < fabs(motorUpper)) && (fabs(back_right_motor.get_position()) > fabs(motorLower)))) &&
      (!((fabs(back_left_motor.get_position()) < fabs(motorUpper)) && (fabs(back_left_motor.get_position()) > fabs(motorLower)))))  {
      // Continue running this loop as long as the motor is not within +-5 units of its goal
      pros::delay(2);
